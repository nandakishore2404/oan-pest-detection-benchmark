...
from benchmark.telemetry import (
    ...
    get_verified_accuracy_evidence,
    get_initiative_evidence
)
...
def render_screen_telemetry(is_subtab: bool = False):
    ...
    # 2. VERIFIED EVIDENCE: Initiative-by-Initiative Impact (evidence-tagged, no fabricated trend)
    st.markdown(f"""
        ...
        <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 10px;">
            <div>
                <div style="font-family: var(--font-display); font-weight: 800; font-size: 16px; color: {COLOR_TEXT};">
                    Verified Evidence: What Has Actually Been Measured
                </div>
                <div style="font-size: 12px; color: {COLOR_MUTED_LIGHT}; margin-top: 4px; line-height: 1.5;">
                    There is no verified single accuracy trend across the full project history -- no comparable
                    before/after numbers exist for "Day 0" through "Sprint 2" on one consistent dataset and
                    methodology. What follows is every number independently re-derived from raw images and raw
                    model files, plus every remediation initiative with a real, traceable before/after state.
                    Nothing below is estimated or interpolated.
                </div>
            </div>
            <div>
                <span class="trust-pill" style="border-color: {COLOR_ACCENT}; color: {COLOR_ACCENT}; font-size: 11px;">
                    {svg_checkmark(COLOR_ACCENT, 12)} Evidence-Tagged
                </span>
            </div>
        </div>
    </div>
    ...
    # 2A. VERIFIED ACCURACY EVIDENCE TABLE + CHART
    acc_evidence = get_verified_accuracy_evidence()
    st.markdown(f"""
    <div style="font-family: var(--font-display); font-weight: 700; font-size: 13px; color: {COLOR_TEXT}; margin-bottom: 6px;">
        1. Every accuracy figure independently re-derived this engagement
    </div>
    """, unsafe_allow_html=True)
    df_acc = pd.DataFrame([{
        "Model": r["model"],
        "Test set": r["test_set"],
        "n": r["n"],
        "Accuracy": f"{r['accuracy_pct']:.2f}%",
        "Status": r["status"],
        "Note": r["note"]
    } for r in acc_evidence])
    st.dataframe(df_acc, use_container_width=True)

    fig_acc = go.Figure(data=[go.Bar(
        x=[f"{r['model']} \u2014 {r['test_set'][:26]}" for r in acc_evidence],
        y=[r["accuracy_pct"] for r in acc_evidence],
        marker_color="#22c08a",
        text=[f"{r['accuracy_pct']:.1f}%" for r in acc_evidence],
    ...
    fig_acc.update_xaxes(gridcolor="#1c2620", zerolinecolor="#1c2620", tickangle=-30)
    fig_acc.update_yaxes(gridcolor="#1c2620", zerolinecolor="#1c2620", range=[0, 100])
    st.plotly_chart(fig_acc, use_container_width=True)

    st.markdown("<div style='height: 12px;'></div>", unsafe_allow_html=True)

    # 2B. INITIATIVE-BY-INITIATIVE BEFORE/AFTER TABLE
    st.markdown(f"""
    <div style="font-family: var(--font-display); font-weight: 700; font-size: 13px; color: {COLOR_TEXT}; margin-bottom: 6px;">
        2. Remediation initiatives with a real, traceable before/after
    </div>
    """, unsafe_allow_html=True)
    init_evidence = get_initiative_evidence()
    df_init = pd.DataFrame([{
        "Initiative": r["initiative"],
        "Before": r["before"],
        "After": r["after"]
    } for r in init_evidence])
    st.dataframe(df_init, use_container_width=True)

    st.warning(
        "CLAHE contrast correction is now wired into the live inference pipeline and runs error-free, "
        "but its effect on accuracy has not yet been re-measured -- the 51.7% and 59.52% field-accuracy "
        "figures above were measured before CLAHE was active.",
        icon="\u26a0\ufe0f"
    )
